...
(async () => {
    ...
    // Combine messages into a single info box
    const message = [
        inputFolder ? `Input folder: ${inputFolder}` : null,
        outputFolder ? `Output folder: ${outputFolder}` : null
    ].filter(Boolean).join('\n');

    if (message) renderInfo(message);
